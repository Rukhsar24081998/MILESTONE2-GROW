...
**Exit criteria:** `manifest.yaml` has exactly 5 URLs; `schemes.json` matches; README documents closed-corpus policy.
...
|----|----------|----------|-------------------|
| P0-EC-01 | P0 | `manifest.yaml` lists 6+ URLs (AMC/AMFI URL added) | Reject at review/CI: fail if `len(allowed_urls) != 5` |
| P0-EC-02 | P0 | `manifest.yaml` lists only 4 URLs (one scheme missing) | Block Phase 1 start; document count must be 5 |
...
| P0-EC-04 | P1 | Extra Groww URL (e.g. HDFC Large Cap) added “for completeness” | Remove from manifest; only locked five URLs allowed |
| P0-EC-05 | P2 | Team assumes problem-statement “15–25 URLs” still applies | README + manifest must state **5-URL closed corpus** override |
...
| P0-EC-09 | P1 | Typos in slug (`hdfc-midcap` vs `hdfc-mid-cap`) | Validation script compares against known five slugs |
...
| P0-EC-14 | P1 | Five schemes in JSON but six entries in manifest | Align counts; one document per scheme URL |
...
| ID | Severity | Scenario | Expected behavior |
|----|----------|----------|-------------------|
| P0-EC-20 | P2 | Reviewer expects ELSS in cohort | Document in README: no ELSS in closed five; thematic + commodities cover diversity |
| P0-EC-21 | P3 | “Large-cap” mentioned in problem statement but not in five schemes | Note as known scope gap; do not add URLs without architecture change |
| P0-EC-22 | P2 | `document_type` not `groww_scheme_page` in manifest | Standardize all five entries to `groww_scheme_page` |
...
| ID | Severity | Scenario | Expected behavior |
|----|----------|----------|-------------------|
| P0-EC-23 | P1 | README omits closed-corpus policy | Exit criteria fail; add explicit “5 URLs only” section |
...
- [ ] `allowed_urls.length === 5`
- [ ] Every manifest URL ∈ known five-slug set
